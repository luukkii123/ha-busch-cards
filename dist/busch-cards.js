...
/* ────────────────────────────────────────────────────────────────────────────
 * busch-timeline-card — Google-Timeline-Nachbau für die Local-Track-Integration
 *
...
function injectLeafletCssHead() {
  if (document.getElementById("busch-leaflet-css")) return;
  const style = document.createElement("style");
  style.id = "busch-leaflet-css";
...
class BuschTimelineCard extends HTMLElement {
  static getConfigElement() {
    return document.createElement("busch-timeline-card-editor");
  }

  static getStubConfig(hass) {
    const entity = Object.keys(hass?.states || {}).find((id) => id.startsWith("person."));
    return { type: "custom:busch-timeline-card", entity: entity || "" };
  }
  ...
  setConfig(config) {
    if (!config || !config.entity) {
      throw new Error("busch-timeline-card: 'entity' fehlt");
    }
    const domain = String(config.entity).split(".")[0];
    if (domain !== "person" && domain !== "device_tracker") {
      throw new Error("busch-timeline-card: 'entity' muss ein person.* oder device_tracker.* sein");
    }
    this._config = {
      height: 320,
      stay_radius_m: 150,
...
class BuschTimelineCardEditor extends HTMLElement {
  setConfig(config) {
    this._config = config || {};
    this._render();
  }
...
customElements.define("busch-timeline-card", BuschTimelineCard);
customElements.define("busch-timeline-card-editor", BuschTimelineCardEditor);

window.customCards.push({
  type: "busch-timeline-card",
  name: "Busch Timeline",
